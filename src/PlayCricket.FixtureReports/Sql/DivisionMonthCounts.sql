-- Reporting-month result counts per league division.
-- Ported from SSRS.sp_Leagues_Divisions_ThisMonth (current year only).
SELECT
...
WHERE f.match_date BETWEEN @MonthStart AND @MonthEnd
  ...
  AND f.Division_Of_Fixture NOT LIKE '%Test%'
