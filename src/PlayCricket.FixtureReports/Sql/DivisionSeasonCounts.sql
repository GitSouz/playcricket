-- Season-to-date result counts per league division for the report year.
-- Ported from SSRS.sp_Leagues_Division_SeasonTD (current season window from
-- lkp.Season, months capped at the reporting month).
SELECT
    CASE f.League_ID WHEN 8205 THEN 15363 ELSE f.League_ID END AS LeagueId,
    f.Division_Of_Fixture AS Division,
...
WHERE MONTH(f.match_date) <= @ReportMonthNumber
  AND f.Year_Of_Fixture = @ReportYear
  AND f.Result IS NOT NULL
  AND f.League_ID > 0
  AND (f.Home_Club_Id NOT IN (9195, 11477, 8141)
       OR f.Away_Club_id NOT IN (9195, 11477, 8141))
GROUP BY CASE f.League_ID WHEN 8205 THEN 15363 ELSE f.League_ID END,
         f.Division_Of_Fixture;
